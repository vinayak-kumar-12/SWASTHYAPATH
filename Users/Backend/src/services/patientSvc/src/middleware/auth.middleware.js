...
const authenticateToken = (req, res, next) => {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return next(new UnauthorizedError("Access token required in Authorization header", "TOKEN_REQUIRED"));
  }
  ...
  try {
    ...
    if (!userId) {
      return next(new UnauthorizedError("Token subject is missing", "INVALID_TOKEN"));
    }
